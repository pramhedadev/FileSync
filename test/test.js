const FileSync = artifacts.require('./FileSync.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('FileSync', ([deployer, uploader]) => {
  let fileSync

  before(async () => {
    fileSync = await FileSync.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await fileSync.address
    ...
    it('has a name', async () => {
      const name = await fileSync.name()
      assert.equal(name, 'FileSync')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await fileSync.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await fileSync.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await fileSync.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await fileSync.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await fileSync.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await fileSync.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await fileSync.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await fileSync.files(fileCount)
